...
def load_checkpoint_with_key_matching(model, checkpoint_path, device):
    ...
    # キー変換前の詳細情報
    print(f"\n=== 変換前の詳細分析 ===")
    ckpt_keys_sample = list(ckpt.keys())[:10]
    model_keys_sample = list(model.state_dict().keys())[:10]
    
    print("チェックポイントのキー例（最初の10個）:")
    for i, key in enumerate(ckpt_keys_sample):
        print(f"  {i+1}: {key}")
    
    print("\nモデルのキー例（最初の10個）:")
    for i, key in enumerate(model_keys_sample):
        print(f"  {i+1}: {key}")
    ...
    print("変換後のチェックポイントキー例（最初の10個）:")
    converted_keys_sample = list(ckpt_converted.keys())[:10]
    for i, key in enumerate(converted_keys_sample):
        print(f"  {i+1}: {key}")
    
    model_keys = set(model.state_dict().keys())
    ckpt_keys = set(ckpt_converted.keys())
    
    print(f"\nモデルのキー数: {len(model_keys)}")
    ...
    if len(common_keys) > 0:
        ...
        print("⚠️ 一致するキーが見つかりません！")
        
        # より詳細な分析
        print("\n=== 詳細分析 ===")
        print("モデルキーのパターン分析:")
        model_patterns = {}
        for key in list(model_keys)[:20]:
            parts = key.split('.')
            pattern = '.'.join(parts[:2]) if len(parts) >= 2 else key
            model_patterns[pattern] = model_patterns.get(pattern, 0) + 1
        
        for pattern, count in sorted(model_patterns.items()):
            print(f"  {pattern}: {count}個")
        
        print("\nチェックポイントキーのパターン分析:")
        ckpt_patterns = {}
        for key in list(ckpt_keys)[:20]:
            parts = key.split('.')
            pattern = '.'.join(parts[:2]) if len(parts) >= 2 else key
            ckpt_patterns[pattern] = ckpt_patterns.get(pattern, 0) + 1
        
        for pattern, count in sorted(ckpt_patterns.items()):
            print(f"  {pattern}: {count}個")
    ...
    if len(common_keys) > 0:
        ...
        print("❌ 互換性のあるキーが見つからないため、ランダム初期化で続行します")
        return False
