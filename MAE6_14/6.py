...
class ImprovedEgo4DFlashDataset(Dataset):
    ...
    def spatial_augmentation(self, video):
        """Enhanced spatial augmentation - simplified for consistency"""
...
def get_args_parser():
    ...
    # Model - Flash Attention をデフォルトで有効化
    parser.add_argument('--grad-checkpointing', action='store_true', dest='use_grad_checkpointing')
    parser.add_argument('--use-flash-attn', action='store_true', dest='use_flash_attn', default=True)  # デフォルトで有効
    parser.add_argument('--disable-flash-attn', action='store_false', dest='use_flash_attn')  # 無効化オプション追加
    ...
    parser.add_argument('--eval-freq', default=2, type=int)  # 評価頻度も調整（10エポックなので2エポックごと）
...
def main(args):
    ...
    # Setup device - CUDA_VISIBLE_DEVICES 環境変数を優先
    if torch.cuda.is_available():
        # CUDA_VISIBLE_DEVICES が設定されている場合、そのデバイスを使用
        visible_devices = os.environ.get('CUDA_VISIBLE_DEVICES', None)
        if visible_devices:
            device = torch.device("cuda:0")  # CUDA_VISIBLE_DEVICES使用時は常に0
            logger.info(f"🎮 Using GPU via CUDA_VISIBLE_DEVICES={visible_devices} -> cuda:0")
        elif args.gpu is not None:
            device = torch.device(f"cuda:{args.gpu}")
            torch.cuda.set_device(args.gpu)
            logger.info(f"🎮 Using specific GPU: cuda:{args.gpu}")
        else:
            device = torch.device("cuda")
            logger.info(f"🎮 Using default CUDA device")
    else:
    ...
    if device.type == 'cuda':
        ...
        logger.info(f"🔥 GPU Info: {gpu_name} ({gpu_memory:.1f}GB) - Device {current_device}")
        
        # Show CUDA_VISIBLE_DEVICES info if set
        if 'CUDA_VISIBLE_DEVICES' in os.environ:
            logger.info(f"🌍 Environment: CUDA_VISIBLE_DEVICES={os.environ['CUDA_VISIBLE_DEVICES']}")
    else:
        logger.warning("⚠️  Running on CPU - this will be very slow!")
    ...
    try:
        train_transform = volume_transforms.Compose([
            volume_transforms.Resize((224, 224)),  # 訓練時も224x224に統一
    ...
    early_stopping = EarlyStopping(patience=5, min_delta=0.001)  # 忍耐値も短縮（10エポックに合わせて）
    best_f1 = 0.0
...
if __name__ == '__main__':
    ...
    print("⚡ QUICK TRAINING: Flash Attention ON, Batch Size 16, Epochs 10, LR 2e-4")
    
    # Show GPU environment info
    if 'CUDA_VISIBLE_DEVICES' in os.environ:
        print(f"🎮 GPU Environment: CUDA_VISIBLE_DEVICES={os.environ['CUDA_VISIBLE_DEVICES']}")
    else:
        print("🎮 GPU Environment: Using default CUDA devices")
    ...
    logger.info("🔧 Enhanced training with QUICK TRAINING defaults:")
    logger.info("   ⚡ Flash Attention: ENABLED by default")
    logger.info("   📦 Batch Size: 16 (optimized for performance)")
    logger.info("   🔄 Epochs: 10 (quick training for testing)")
    logger.info("   📈 Learning Rate: 2e-4 (tuned for stability)")
    logger.info("   🕐 Warmup: 2 epochs (optimized for short training)")
    logger.info("   📊 Eval Frequency: Every 2 epochs")
