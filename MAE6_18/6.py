...
def ultra_test_time_augmentation(model, video, device):
    ...
    color_transforms = [
        ...
        lambda x: torch.clamp(x * 1.2 - 0.1, 0, 1),  # Contrast +
        lambda x: torch.clamp(x * 0.8 + 0.1, 0, 1),  # Contrast -
    ]
    
    # 時間変換
    temporal_transforms = [
        lambda x: torch.roll(x, shifts=1, dims=2),  # Temporal shift +1
        lambda x: torch.roll(x, shifts=-1, dims=2),  # Temporal shift -1
        lambda x: x[:, torch.randperm(x.shape[1])],  # Frame shuffle (light)
    ]
    
    # 空間変換  
    spatial_transforms = [
        lambda x: F.interpolate(x, scale_factor=1.1, mode='trilinear', align_corners=False)[:, :, :, :224, :224],  # Zoom in
        lambda x: F.pad(F.interpolate(x, scale_factor=0.9, mode='trilinear', align_corners=False), 
                       (12, 12, 12, 12), mode='reflect'),  # Zoom out + pad
    ]
    
    # ノイズ・ブラー
    noise_transforms = [
        lambda x: torch.clamp(x + torch.randn_like(x) * 0.02, 0, 1),  # Gaussian noise
        lambda x: x + torch.sin(torch.arange(x.shape[2], device=x.device).float() * 0.1).view(1, 1, -1, 1, 1) * 0.05,  # Temporal noise
    ]
    
    # 全変換をまとめる
    all_transforms = base_transforms + color_transforms + temporal_transforms + spatial_transforms + noise_transforms
    
    # 重み設定（基本変換に高い重み）
    weights = torch.tensor([
        0.20, 0.12, 0.08, 0.08,  # base (4)
        0.06, 0.06, 0.06, 0.06,  # color (4) 
        0.05, 0.05, 0.04,        # temporal (3)
        0.04, 0.04,              # spatial (2)
        0.03, 0.03               # noise (2)
    ]).to(device)
    
    # 予測実行
    for i, (transform, weight) in enumerate(zip(all_transforms, weights)):
        try:
            # 変換適用
            if i < 4:  # 基本変換は直接適用
                ...
                with torch.no_grad():
                    ...
                    if transformed_video.shape != video.shape:
                        ...
                        transformed_video = F.interpolate(
                            transformed_video.view(-1, C, H, W), 
            ...
            # フォールバック: オリジナル予測
            print(f"TTA transform {i} failed: {e}")
            try:
                with torch.no_grad():
                    with autocast():
                ...
                # 最後の手段: ゼロ予測
                predictions.append(torch.zeros_like(predictions[0] if predictions else torch.zeros(1, 58).to(device)) * weight)
