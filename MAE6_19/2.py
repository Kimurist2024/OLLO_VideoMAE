...
class VideoInference:
    ...
    def draw_predictions(self, frame, predictions):
        ...
        # Semi-transparent background parameters
        overlay = frame.copy()
        ...
        bg_height = len(display_predictions) * 35 + 20
        cv2.rectangle(overlay, 
                     (padding - 5, 10),
                     (padding + max_width + 25, bg_height + 10),
                     (0, 0, 0), -1)
        ...
        for i, (label, prob) in enumerate(display_predictions):
            ...
            # Determine text color based on label prefix with brighter colors
            if label.startswith('r_'):
                ...
                text_color = (0, 255, 255)  # Yellow for change
            elif label.startswith('s_'):
                text_color = (255, 0, 255)  # Magenta for search
            elif label.startswith('m_'):
                text_color = (0, 165, 255)  # Orange for moving
            elif label.startswith('t_'):
                text_color = (255, 255, 0)  # Cyan for turning
            else:
