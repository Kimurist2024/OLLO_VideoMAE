...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.use_flash_attn:
            x = F.scaled_dot_product_attention(q, k, v, dropout_p=0.0).transpose(1, 2).reshape(B, N, self.all_head_dim)
        else:
            q = q * self.scale
...
class VisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=58, embed_dim=768, depth=12, num_heads=12,
        ...
        self.blocks = nn.ModuleList([
            Block(embed_dim, num_heads, mlp_ratio, qkv_bias, qk_scale, drop_rate, attn_drop_rate, dpr[i], init_values, norm_layer=norm_layer, act_layer=nn.GELU, use_flash_attn=use_flash_attn)
