...
import sys
sys.path.append("/home/ollo/avion")  # avion モジュールのパスを追加

from modeling_videomae import pretrain_videomae_base_patch16_224

def get_model(num_classes=58):
    model = pretrain_videomae_base_patch16_224()
    model.head = nn.Linear(model.head.in_features, num_classes)  # 出力層を自分の分類数に合わせる
    return model
...
def train_model_interactive():
    ...
    model = get_model(num_classes=58).to(device)
