...
def train_model_interactive():
    ...
    val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=2, shuffle=False, num_workers=4, pin_memory=True)
    ...
    for epoch in range(20):
        ...
        model.train()
        epoch_loss = 0.0
        for videos, targets in tqdm(train_loader, desc=f"Train Epoch {epoch+1}"):
        ...
        all_targets = []
        with torch.no_grad():
            for videos, targets in tqdm(val_loader, desc=f"Val Epoch {epoch+1}"):
                videos, targets = videos.to(device), targets.to(device)
                with torch.cuda.amp.autocast():
                    outputs = model(videos)
        ...
        if f1 > best_f1:
            ...
            print(f"[Epoch {epoch+1}] New best model found with F1: {f1:.4f}")

    if best_model is not None:
        torch.save(best_model, "videomae_best_f1.pth")
        print(f"Best F1 score: {best_f1:.4f}. Model saved to 'videomae_best_f1.pth'.")
    else:
        print("No improvement in F1 score was observed during training.")
