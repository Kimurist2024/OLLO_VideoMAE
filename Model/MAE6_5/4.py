...
def train_model_interactive():
    ...
    for epoch in range(20):
        ...
        print(f"[DEBUG] val_loader length: {len(val_loader)}")

        with torch.no_grad():
            for batch_idx, (videos, targets) in enumerate(val_loader):
                print(f"[DEBUG] Running val batch {batch_idx+1}/{len(val_loader)}")
                videos, targets = videos.to(device), targets.to(device)
                with autocast():
                    outputs = model(videos)
        ...
        if len(all_preds) == 0:
            print("[WARNING] No predictions were generated during validation.")
            continue
        ...
        print(f"[DEBUG] all_targets shape: {all_targets.shape}")
        print(f"[DEBUG] Sample predictions:\n{all_preds[:2]}")
        print(f"[DEBUG] Sample targets:\n{all_targets[:2]}")
