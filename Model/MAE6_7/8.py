...
def main():
    ...
    vit_base_ckpt_path = "/path/to/mae_pretrain_vit_base.pth"
    ...
    if os.path.exists(vit_base_ckpt_path):
        print(f"Loading ViT-Base pre-trained weights from {vit_base_ckpt_path}")
        pretrained_state = torch.load(vit_base_ckpt_path, map_location=device)
        if "model" in pretrained_state:
            pretrained_state = pretrained_state["model"]
        model_state = model.state_dict()
        matched_weights = {k: v for k, v in pretrained_state.items() if k in model_state and v.shape == model_state[k].shape}
        print(f"Matched {len(matched_weights)} / {len(model_state)} layers from ViT-Base checkpoint")
