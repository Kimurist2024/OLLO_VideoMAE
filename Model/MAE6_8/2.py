...
def main():
    ...
    if os.path.exists(converted_ckpt_path):
        ckpt_raw = torch.load(converted_ckpt_path, map_location=device)
        ckpt = ckpt_raw.get("model", ckpt_raw)
        model_keys = set(model.state_dict().keys())
        ckpt_keys = set(ckpt.keys())
        print("モデルのキー数:", len(model_keys))
        print("チェックポイントのキー数:", len(ckpt_keys))
        print("差分:", model_keys.symmetric_difference(ckpt_keys))
