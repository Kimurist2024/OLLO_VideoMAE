...
def main():
    ...
    for epoch in range(300):
        pbar = tqdm(dataloader, desc=f"Epoch {epoch+1}/300")
        epoch_loss = 0.0
        num_batches = 0
        ...
        # 300エポック目のみ保存
        if epoch + 1 == 300:
            torch.save(model.state_dict(), "/home/ollo/pretrain_epoch300.pth")
            print(f"✓ Saved model for epoch 300 with average loss: {avg_loss:.6f}")
            print(f"✓ Model saved to: /home/ollo/pretrain_epoch300.pth")
        
        # 進捗表示（10エポックごと）
        if (epoch + 1) % 10 == 0:
            print(f"Epoch {epoch+1}/300 completed. Average loss: {avg_loss:.6f}")
